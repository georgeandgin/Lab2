 ...
 */
function storytime_get_svg( $icon = null ) {
 ...
 */
 function storytime_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	if ( 'primary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . '<span class="sub-menu-icon">' . storytime_get_svg( 'expand' ) . '</span>';
			}
		}
	}
...
add_filter( 'nav_menu_item_title', 'storytime_dropdown_icon_to_menu_link', 10, 4 ); 
 ...
 * @return string $svg SVG markup.
 */
function storytime_get_social_svg( $icon = null ) {
	// Return early if no icon was defined.
	if ( empty( $icon ) ) {
		return;
	}
 ...
 */
function storytime_social_icons_menu( $item_output, $item, $depth, $args ) {

	// Get supported social icons.
	$social_icons = storytime_supported_social_icons();
	...
	if ( 'top-social' === $args->theme_location ) {
		...
		$item_output = str_replace( $args->link_after, '</span>' . storytime_get_social_svg( $icon ), $item_output );
	}

	return $item_output;
}
add_filter( 'walker_nav_menu_start_el', 'storytime_social_icons_menu', 10, 4 );
 ...
 */
function storytime_supported_social_icons() {
